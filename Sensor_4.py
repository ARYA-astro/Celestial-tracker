...
def get_orientation(offset_mx, offset_my):
...
def cure_heading_displacement(heading, target_az):
    # Calculate the difference between the current heading and the target azimuth
    displacement = heading - target_az
    
    # Normalize the displacement to be within the range of -180 to 180 degrees
    if displacement > 180:
        displacement -= 360
    elif displacement < -180:
        displacement += 360
        
    return displacement+1.3
...
if __name__ == "__main__":
    try:
        ...
        while True:
            ...
            print(f"Displacement from Target: Altitude Error: {actual_pitch - target_alt:+5.1f}° | Azimuth Error: {cure_heading_displacement(heading, target_az):+5.1f}°")
